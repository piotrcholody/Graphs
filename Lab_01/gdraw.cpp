...
/*
void Gdraw::on_actionImport_Adjascency_triggered()
{
    bool** mainTab = this->curr_graph->getMatrix();
    int n = this->curr_graph->getN();
    bool** tab = GConverter::copyTab(mainTab, n, n);

    for(int i = 0; i<n; i++)
        for(int j = 0; j<n; j++)
        {
            tab[i][j] = (ui->tableWidget->item(i,j)->text()).toInt();
        }

    if (GConverter::checkAdjacency(tab, n))
    {
        this->curr_graph->addTab(tab);
        GConverter::deleteTab(mainTab, n);
    }
    else
        GConverter::deleteTab(tab, n);
}

void Gdraw::on_actionImportIncidenceT_triggered()
{
    int n = this->curr_graph->getN();
    bool** tab = this->curr_graph->getTab();
    ...
    for(int i=0; i<n; i++)
        for(int j=0; j<l; j++)
            tab2[i][j] = (ui->tableWidget_2->item(i,j)->text()).toInt();

    if (GConverter::checkIncidence(tab2, n, l))
    {
        matrix->addTab(tab2);
        delete this->curr_graph;
        this->curr_graph = GConverter::incidenceToAdjacency(matrix);
    }
    delete matrix;
}

void Gdraw::on_actionImportAdjascency_List_triggered()
{
    int n = this->curr_graph->getN();
    int maxL = Graf::maksIloscKrawedzi(n);
    AdjacencyList *list = new AdjacencyList(n);
    std::vector<int>* tab = list->getTab();

    for(int i=0; i<n; i++)
        for(int j=0;  j<maxL; j++)
        {
            QTableWidgetItem* tmpItem = ui->tableWidget_3->item(i,j);
            if (!(!tmpItem || tmpItem->text().isEmpty()))
                tab[i].push_back((tmpItem->text()).toInt());
        }

    if (GConverter::checkList(tab, n))
    {
        delete this->curr_graph;
        //this->curr_graph = GConverter::listToMatrix(list);
        this->curr_graph = GConverter::listToMatrix(GConverter::incidenceToList(GConverter::listToIncidence(list)));
    }
